...
struct tab_fusfus{
    float text_opacity, text_opacity1, text_opacity2, text_opacity3, rect_opacity, rect_opacity1, rect_opacity2, rect_opacity3, rect_opacity4;
    
};

bool ImGui::ImGui_GOD_TAB(bool boolean, const char* icon, const char* label,  const ImVec2& size_arg){
    ...
    static std::map <ImGuiID, tab_fusfus> anim;
    ...
    it_anim->second.text_opacity1 = ImLerp(it_anim->second.text_opacity1, (boolean ? menu[0] : 1.0f), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity2 = ImLerp(it_anim->second.text_opacity2, (boolean ? menu[1] : 1.0f), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity3 = ImLerp(it_anim->second.text_opacity3, (boolean ? menu[2] : 1.0f), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, (boolean ? 1.0f : 0.5f), 0.13f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.rect_opacity1 = ImLerp(it_anim->second.rect_opacity1, (boolean ? (size.x)/2 : 0.0f), 0.09f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.rect_opacity2 = ImLerp(it_anim->second.rect_opacity2, (boolean ? (size.y)/2 : 0.0f), 0.09f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.rect_opacity = ImLerp(it_anim->second.rect_opacity, (boolean ? 1.0f : 0.0f), 0.50f * (1.0f - ImGui::GetIO().DeltaTime));
    
    window->DrawList->AddRectFilled(ImVec2( bb.GetCenter().x - it_anim->second.rect_opacity1, bb.GetCenter().y - it_anim->second.rect_opacity2), ImVec2( bb.GetCenter().x + it_anim->second.rect_opacity1, bb.GetCenter().y + it_anim->second.rect_opacity2), ImColor( 14/255.f,  14/255.f, 15/255.f, it_anim->second.rect_opacity), 6.0f);
    window->DrawList->AddRect(ImVec2( bb.GetCenter().x - it_anim->second.rect_opacity1, bb.GetCenter().y - it_anim->second.rect_opacity2), ImVec2( bb.GetCenter().x + it_anim->second.rect_opacity1, bb.GetCenter().y + it_anim->second.rect_opacity2), ImColor( 27/255.f,  29/255.f, 32/255.f, it_anim->second.rect_opacity), 6.0f, ImDrawFlags_RoundCornersAll, 4.0f);
    ImGui::PushFont(TAB_FONTS); 
    window->DrawList->AddText(ImVec2(bb.Min.x + 20,bb.GetCenter().y - (29/2)), ImColor(it_anim->second.text_opacity1, it_anim->second.text_opacity2, it_anim->second.text_opacity3, it_anim->second.text_opacity), icon);
    ImGui::PopFont();
    window->DrawList->AddText( bb.Min + ImVec2(60, bb.GetSize( ).y / 2.f - CalcTextSize( label ).y / 2.f ), ImColor(1.0f, 1.0f, 1.0f, it_anim->second.text_opacity), label );
    
    /*
    PushStyleColor(ImGuiCol_Text, ImVec4(1.0f,1.0f,1.0f, it_anim->second.text_opacity));
...
{
    static std::map<bool*, float> anim2;
  static std::map<ImGuiID, check_state> anim;
    anim2[v] = lerp(anim2[v], (float)*v, GImGui->IO.DeltaTime * 10);
   ...
   // static std::map<ImGuiID, check_state> anim;
    ...
    ImU32 check_col = ImColor(14, 14, 14, 240);
    ImColor outlinecolor = collerp(GetColorU32(ImGuiCol_CheckMark), GetColorU32(ImGuiCol_CheckMark), anim2[v]);
    ImColor fillcolor = collerp(check_col, GetColorU32(ImGuiCol_CheckMark), anim2[v]);
    ...
    //AddRadialGradient(window->DrawList, check_bb.Min + ImVec2(pad2, pad2), 35 * style.DpiScale, ImColor(129, 137, 183, (int)(anim[v] * 200.0f)), ImColor(129, 137, 183,0));
    ...
    if (mixed_value)
    {
        ...
        ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 3.6f)), ImMax(1.0f, IM_FLOOR(square_sz / 3.0f)));
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, check_col, style.FrameRounding);
    }


    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad) + ImVec2(square_sz / 2, square_sz / 2) * (1.0f - anim2[v]), check_col, (square_sz - pad * 1.7f) * anim2[v]);
...
namespace Memek { 
ImVec4 Asu(float r, float g, float b, float a) { return ImVec4(r / 155.0, g / 155.0, b / 155.0, a / 155.0); }
}

static ImVec4 active = Memek::Asu(0, 0, 0, 255); //Colour Disbale Button
static ImVec4 inactive = Memek::Asu(0, 0, 0, 255); //Colour Enable Button
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // Render shape
    const ImU32 frame_col = GetColorU32(hovered ? inactive : active);
    ...
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? inactive : active);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImVec2 lineend = ImVec2(lerp(grab_bb.Min.x, grab_bb.Max.x, 0.5f), lerp(grab_bb.Min.y, grab_bb.Max.y, 3.5f));
    ImVec2 backgroundend = ImVec2(lerp(grab_bb.Min.x, grab_bb.Max.x, 0.5f), lerp(grab_bb.Min.y, grab_bb.Max.y, 3.5f));
    ImVec2 linesize = ImVec2(0.5f, 2 * style.DpiScale);

    window->DrawList->AddRectFilled(linestart - linesize, backgroundend + linesize, ImColor(60,60,60), style.GrabRounding);
    window->DrawList->AddRectFilled(linestart - linesize, lineend + linesize, ImColor(129, 137, 183,255), style.GrabRounding);
    AddRadialGradient(window->DrawList, lineend, 25 * style.DpiScale, ImColor(255, 255, 255, (int)(anim1[p_data] * 170.0f)), ImColor(0, 255, 0, 255));
    window->DrawList->AddCircleFilled(lineend, 5 * style.DpiScale * (anim1[p_data] / 2.0f + 1.0f), GetColorU32(ImGuiCol_CheckMark));
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, -2.5));
...
bool ImGui::SliderScalar2(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
...
const float value_width = 60.0f;
const float spacing = 5.0f;
const float label_width = 100.0f;

bool has_visible_label = !(label[0] == '#' && label[1] == '#');

float slider_start_x = window->DC.CursorPos.x + (has_visible_label ? label_width + spacing : 0.0f);
float slider_end_x = window->WorkRect.Max.x - value_width - spacing;

    const ImVec2 slider_pos_min(slider_start_x, window->DC.CursorPos.y);
    const ImVec2 slider_pos_max(slider_end_x, window->DC.CursorPos.y + 20 + label_size.y);

    const ImRect frame_bb(slider_pos_min, slider_pos_max);
    const ImRect total_bb(window->DC.CursorPos, ImVec2(window->WorkRect.Max.x, slider_pos_max.y));
    ImRect grab_bb;
    ...
    {
        const bool is_clamp_input = (flags & ImGuiSliderFlags_AlwaysClamp) != 0;
        return TempInputScalar(frame_bb, id, label, data_type, v, format, is_clamp_input ? v_min : NULL, is_clamp_input ? v_max : NULL);
    }

    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);

    if (SliderBehavior(frame_bb, id, data_type, v, v_min, v_max, format, flags, &grab_bb))
        MarkItemEdited(id);

    ImVec2 fill_end = ImVec2(grab_bb.Min.x + 10, frame_bb.Min.y + 25);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 15, frame_bb.Min.y + 28), fill_end, ImColor(93, 128, 255, int(255 * GetStyle().Alpha)), 3);


    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 15, frame_bb.Min.y + 28), ImVec2(frame_bb.Max.x - 18, frame_bb.Max.y - 7), ImColor(43, 43, 49, int(255 * GetStyle().Alpha)), 3);

    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 15, frame_bb.Min.y + 32), ImVec2(grab_bb.Max.x - 15, grab_bb.Max.y - 5), ImColor(178, 182, 255, int(255 * GetStyle().Alpha)), 3);
    }

    RenderFrame2(grab_bb.Max - ImVec2(14 , 9) - ImVec2(0, 9), grab_bb.Max + ImVec2(4, 8) - ImVec2(0, 8), GetColorU32(ImVec4(43 / 255.f, 43 / 255.f, 49 / 255.f, 255 / 255.f)), true, 2.5f, true, ImColor(178, 182, 255));
    ImGui::GetWindowDrawList()->AddCircleFilled((grab_bb.Max - ImVec2(14, 9) - ImVec2(0, 9) + grab_bb.Max + ImVec2(4, 8) - ImVec2(0, 8)) / 2, 2, ImColor(178, 182, 255));

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
    ImVec2 label_buffer_size = CalcTextSize(value_buf, value_buf_end, true);

    window->DrawList->AddRectFilled(ImVec2(window->WorkRect.Max.x - value_width + 5, frame_bb.Min.y + 32), ImVec2(window->WorkRect.Max.x - 7, frame_bb.Max.y - 7 + 4), ImColor(43, 43, 49, int(255 * GetStyle().Alpha)), 3);

ImVec2 value_box_min = ImVec2(window->WorkRect.Max.x - value_width + 5, frame_bb.Min.y + 28);
ImVec2 value_box_max = ImVec2(window->WorkRect.Max.x - 7, frame_bb.Max.y - 7 + 4);

window->DrawList->AddRectFilled(value_box_min, value_box_max, ImColor(43, 43, 49, int(255 * GetStyle().Alpha)), 4);
window->DrawList->AddRect(value_box_min, value_box_max, ImColor(93, 128, 255, int(255 * GetStyle().Alpha)), 2); 

PushStyleColor(ImGuiCol_Text, ImColor(0, 255, 0, int(255 * GetStyle().Alpha)).operator ImVec4());
ImVec2 text_pos = ImVec2(
    value_box_min.x + (value_box_max.x - value_box_min.x - label_buffer_size.x) * 0.5f,
    value_box_min.y + (value_box_max.y - value_box_min.y - label_buffer_size.y) * 0.5f
);
RenderText(text_pos, value_buf, value_buf_end);
PopStyleColor();

    PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255 * GetStyle().Alpha)).operator ImVec4());
    RenderText(ImVec2(window->DC.CursorPos.x, (frame_bb.Min.y + frame_bb.Max.y) * 0.5f - label_size.y * 0.5f), label);
    PopStyleColor();

    ImGui_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);

    return SliderBehavior(frame_bb, id, data_type, v, v_min, v_max, format, flags, &grab_bb);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const char* buf_display = buf; //-V595
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf,
                                           nullptr, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, col, buf, nullptr, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
...
bool ImGui::VirtualInputEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
   ...
   // RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
    ...
    if (values_count >= values_count_min)
    {
        ...
        const ImU32 col_base = ImColor(255,255,255);
        const ImU32 col_hovered = ImColor(255,255,255);
